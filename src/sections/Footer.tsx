        ...
        <div className="flex flex-col items-start">
          ...
          <div className='flex items-center gap-5 mt-8'>
            {socialMedia.map((icon) => (
              <div key={icon.alt} className='flex justify-center items-center w-12 h-12 bg-white rounded-full hover:shadow-slate-gray hover:scale-125 duration-200'>
            ...
            <div key={section.title}>
              ...
              <ul>
                {section.links.map((link) => (
                  <li key={link.name} className='text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray cursor-pointer'>
                    <a href="">{link.name}</a>
                  </li>
                ))}
              </ul>
